...
const filtered = data.filter(function(item){ return item.ingredients.find(a => a == "citron vert") , item.tags.find(a => a == "sucré")});

 // Version avec includes
// cocktail.ingredients.includes('citron vert') || cocktail.tags.includes('sucré')

const found = data.find(cocktail => cocktail.id === "10");

console.log("avec filter classique↓↓", filtered,"avec find↓↓" , found);